    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "55fr 45fr", gap: 18, height: "100%", overflow: "hidden", paddingBottom: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", overflowY: "auto", paddingBottom: 28, paddingRight: 2 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {REPORTS.map((r) => (
            <div key={r.id} className="card card--hover" style={{ padding: 18 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 10 }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: toneColor[r.tone] }} />
                <span style={{ fontWeight: 600, fontSize: 14 }}>{r.kind}</span>
                <span className="faint mono" style={{ marginLeft: "auto", fontSize: 11 }}>{r.date}</span>
              </div>
              <div className="muted" style={{ fontSize: 13, lineHeight: 1.6 }}>{r.summary}</div>
            </div>
          ))}
        </div>
      </div>
